  ...
  public = list(
    ...
    initialize = function(required.metrics = c("MCC", "PPV")) {
      if (any(is.null(required.metrics),
        ...
        d2mcs.log(message = "Invalid values of required.metrics. Aborting...",
                  level = "FATAL",
    ...
    getFinalPrediction = function(raw.pred, prob.pred, positive.class, negative.class) {
      if (is.null(raw.pred) || !is.list(raw.pred)) {
        d2mcs.log(message = paste0("Raw.pred parameter must be defined as ",
                                   "'list' type. Aborting..."),
                  level = "FATAL",
                  className = class(self)[1],
                  methodName = "getFinalPrediction")
      }

      if (!all(self$getRequiredMetrics() %in% names(raw.pred))) {
        d2mcs.log(message = paste0("Raw.pred parameter must have required metrics. ",
                                   paste(self$getRequiredMetrics(),
                                         collapse = " "), ". Aborting..."),
                  level = "FATAL",
                  className = class(self)[1],
                  methodName = "getFinalPrediction")
      ...
      if (is.null(prob.pred) || !is.list(prob.pred)) {
        d2mcs.log(message = paste0("Prob.pred parameter must be defined as ",
                                   "'list' type. Aborting..."),
                  level = "FATAL",
                  className = class(self)[1],
                  methodName = "getFinalPrediction")
      ...
      if (!all(self$getRequiredMetrics() %in% names(prob.pred))) {
        d2mcs.log(message = paste0("Prob.pred parameter must have required metrics. ",
                                   paste(self$getRequiredMetrics(),
                                         collapse = " "), ". Aborting..."),
                  level = "FATAL",
                  className = class(self)[1],
      ...
      if (is.null(positive.class) || (!is.character(positive.class) && !is.numeric(positive.class))) {
        d2mcs.log(message = "Positive class parameter must be defined. Aborting...",
                  level = "FATAL",
                  className = class(self)[1],
                  methodName = "getFinalPrediction")
      }
      if (is.null(negative.class) || (!is.character(negative.class) && !is.numeric(negative.class))) {
        d2mcs.log(message = "Negative class parameter must be defined. Aborting...",
                  level = "FATAL",
                  className = class(self)[1],
                  methodName = "getFinalPrediction")
      }
